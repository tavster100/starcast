      ...
      <DialogContent className="sm:max-w-[500px] bg-gradient-to-br from-background to-background/90 backdrop-blur-lg border-pink-500/30">
        ...
        <div className="flex flex-col items-center justify-center py-4 text-center">
          ...
          <p className="text-gray-300 mb-4">
            Ai {submissions.length} înregistrări salvate local. În mediul de producție, acestea ar fi trimise prin
            email.
          </p>

          <div className="w-full max-h-60 overflow-y-auto bg-black/20 rounded-md p-4 mb-4">
            {submissions.map((submission, index) => (
              <div key={index} className="mb-4 pb-4 border-b border-gray-700 last:border-0 text-left">
                <p>
                ...
                <p>
                  <strong>Tip:</strong> {submission.formType === "signup" ? "Înscriere" : "Consultație"}
                </p>
                <p>
                  <strong>Data:</strong> {new Date(submission.date).toLocaleString()}
                </p>
              </div>
            ))}
          </div>
